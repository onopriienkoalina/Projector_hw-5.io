...
function editTask(event) {
    // перевіряємо чи клік був на елементі списку
    if (event.target.classList.contains('task')) {
        // отримуємо текст елементу списку
        const taskText = event.target.firstChild.textContent;
        // відображаємо текст в інпуті для введення завдання
...
function updateTaskInLocalStorage(task) {
    ...
    // знаходимо індекс завдання, яке треба оновити
    const taskIndex = tasks.findIndex((item) => item === task);
    // видаляємо завдання зі списку
    tasks.splice(taskIndex, 1);
    // додаємо оновлене завдання в кінець списку
